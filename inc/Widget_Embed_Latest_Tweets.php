...
class Widget_Embed_Latest_Tweets extends WP_Widget {
	...
	public function form( $instance ) {
		...
		if ( is_array( $twitter_oauth_var ) && count( $twitter_oauth_var ) == 2 ) {
            ...
            <p>
                <label
                    for="<?php echo $this->get_field_id( 'lang' ); ?>"><?php _e( 'Language', 'widget-embed-lastest-tweets' ) ?>
                    :</label>
                <input id="<?php echo $this->get_field_id( 'lang' ); ?>"
                       name="<?php echo $this->get_field_name( 'lang' ); ?>"
                       type="text" value="<?php echo $lang; ?>" size="2"/>
                <br/>
                <span
                    class="description"><?php _e( 'Two firsts caractere only. Example : "fr" for french', 'widget-embed-lastest-tweets' ) ?></span>
            </p>
			<?php
	...
	private function text_field( $option, $value, $label ) {
        ...
        <p>
            <label
                for="<?php echo $this->get_field_id( $option ); ?>"><?php echo $label; ?>
                :</label>
            <input class="widefat"
	...
	private function checkbox_field( $option, $value, $label, $description = '' ) {
        ...
        <p>
            <label
                for="<?php echo $this->get_field_id( $option ); ?>"><?php echo $label ?>
                :</label>
            <input id="<?php echo $this->get_field_id( $option ); ?>"
                   name="<?php echo $this->get_field_name( $option ); ?>"
                   type="checkbox" <?php checked( $value ) ?>
	...
	private function select_field( $option, $value, $label, $options, $description = '' ) {
        ...
        <p>
            <label
                for="<?php echo $this->get_field_id( $option ); ?>"><?php echo $label ?>
                :</label>
            <select id="<?php echo $this->get_field_id( $option ); ?>"
                    name="<?php echo $this->get_field_name( $option ); ?>">
				<?php foreach ( $options as $option ) { ?>
                    <option
                        value="<?php echo $option ?>" <?php selected( $option, $value, TRUE ) ?>><?php echo $option ?></option>
				<?php } ?>
            </select>
			<?php
			if ( $description ) {
	...
	private function number_field( $option, $value, $label, $min, $max, $description = '' ) {
        ...
        <p>
            ...
            <input id="<?php echo $this->get_field_id( $option ); ?>"
                   name="<?php echo $this->get_field_name( $option ); ?>"
                   type="number" step="1" min="<?php echo $min; ?>"
                   max="<?php echo $max; ?>"
